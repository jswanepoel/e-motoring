  ...
  public createConnection(request: Request) {
    ...
    xhrConnection.response = xhrConnection.response.pipe(catchError((error: Response) => {
      if ((error.status === 401 || error.status === 403) && (window.location.href.match(/\?/g) || []).length < 2) {
        ...
        1. Auth Guard checks authorized user (e.g. by looking into SessionStorage). 
        2. On 401/403 response you clean authorized user for the Guard (e.g. by removing coresponding parameters in SessionStorage). 
