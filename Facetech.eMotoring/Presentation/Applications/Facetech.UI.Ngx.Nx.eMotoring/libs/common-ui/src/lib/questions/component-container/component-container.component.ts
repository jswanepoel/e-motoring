...
import { DependentOption, QuestionDefinition, ComponentDefinition } from '../question-base.component';


@Component({
  selector: 'ngx-component-container',
...
export class ComponentContainerComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('compRef', { read: ElementRef }) compRef: ElementRef;

  private _isLoading: boolean = false;

  public modelSubscription: Subscription;
  public modelSubject: BehaviorSubject<any>;
  public disabled: boolean = false;
  public hidden: boolean = false;

  public ngOnInit() {
    this.modelSubject = new BehaviorSubject<any>(this.definition.data); //create Behavior Subject of the value in the QuestionDefinition

    //this.modelSubscription = this.modelSubject.subscribe(x => { //Calling .next() on valueSubject will update the questions value
    //  this.form.get(this.question.key).setValue(x);
    //});

    this.toggleHide(this.definition.hidden); //set initial hidden or 
    //this.toggleDisable(this.definition.disabled); //disabled state 

    if (!isNullOrUndefined(this.definition.dependentOptions)) { //if there are dependencies specified then
      this.definition.dependentOptions.forEach(dependentOption => { //for each dependency specified, subscribe to the value        
    ...
    //if (!isNullOrUndefined(this.question.defaultValue)) {
    //  let val = this.getAttrValFromList(this.question.defaultValue);
