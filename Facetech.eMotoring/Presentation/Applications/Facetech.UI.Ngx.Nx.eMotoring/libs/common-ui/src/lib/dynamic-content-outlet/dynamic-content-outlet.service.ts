...
export class DynamicContentOutletService {
  ...
  public async getComponent(componentName: string): Promise<ComponentRef<any>> {
    ...
    try {
      const moduleFactory: NgModuleFactory<any> = await this.moduleLoader.load(modulePath);
      const moduleReference = moduleFactory.create(this.injector);
      const componentResolver: ComponentFactoryResolver = moduleReference.componentFactoryResolver;
      ...
      return this.getDynamicContentErrorComponent(
        `Unable to load module ${modulePath}. Looked up using component: ${componentName}. Error Details: ${error.message}`);
    }
  }

  private getModulePathForComponent(componentName: string) {
    let modulePath: string = null;
    const manifest = DynamicContentOutletMappings.find(m => m.path === componentName);
  ...
  private getDynamicContentErrorComponent(errorMessage: string) {
    const factory = this.componentFactoryResolver.resolveComponentFactory(DynamicContentOutletErrorComponent);
